          ...
          <c:choose>
          	<c:when test="${login == null}">
          		<c:if test="${countLabel == 0}">
            		<input name="Button" type="button" class="buttonMain" onClick="javascript:checkvalidatepublisher('<%=request.getContextPath()%>/admin/adduserinfo.do?PAGE_VALUE=1&ADMIN_MODULE=PUB&METHOD=USER_ADD&LOGIN=${login}');" value="Add Pub Unit">
            	</c:if>
            	<input name="Button2" type="button" class="buttonMain" onClick="javascript:self.close();" value="Close">
            </c:when>
            <c:otherwise>
            	<c:if test="${countLabel == 0}">
            		<input name="Button" type="button" class="buttonMain" onClick="javascript:checkvalidatepublisher('<%=request.getContextPath()%>/admin/adduserinfo.do?PAGE_VALUE=1&ADMIN_MODULE=PUB&METHOD=USER_EDIT&LOGIN=${login}');" value="Add Pub Unit">
            	</c:if>
            	<input name="Button2" type="button" class="buttonMain" onClick="javascript:self.close();" value="Close">
            </c:otherwise>
          </c:choose>
