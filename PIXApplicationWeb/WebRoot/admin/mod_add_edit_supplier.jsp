            ...
            <c:otherwise>
            	<c:choose>
    				<c:when test="${supplierForm.party.activeFlag == 'Y'}">
            			<input name="Button3" type="button" class="buttonMain" onClick="if(validateSupplier()){submitActionForSupplier('<%=request.getContextPath()%>/admin/updatesupplier.do?SAN_VALUE=${san_no}&PAGE_VALUE=${PAGE_VALUE}')}else{return false;}" value="Update">
            		</c:when>
            		<c:otherwise>
            				<input name="Button3" type="button" class="buttonMain" onClick="if(validateSupplier()){submitActionSupplier('<%=request.getContextPath()%>/admin/updatesupplier.do?SAN_VALUE=${san_no}&PAGE_VALUE=${PAGE_VALUE}')}else{return false;}" value="Update">
            		</c:otherwise>
            	</c:choose>
            	<%--<input name="FlagSupplierEdit" type="hidden" value="FlagSupplierEdit"/>--%>
            	<input name="SupplierEditCancel" type="hidden" value="SupplierEditCancel"/>
            	<input name="Button4" type="button" class="buttonMain" onClick="submitSupplierCancelAction('<%=request.getContextPath()%>/admin/listsup.do?PAGE_VALUE=${PAGE_VALUE}')" value="Close">
   			</c:otherwise>
   		  </c:choose>
   		  </td>
            </tr>
