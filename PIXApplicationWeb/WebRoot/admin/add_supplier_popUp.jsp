			...
			<c:choose>
        		<c:when test="${login == null}">
        			<c:if test="${countLabel == 0}">
        				<input name="Button" type="button" class="buttonMain" onClick="javascript:checkvalidate('<%=request.getContextPath()%>/admin/adduserinfo.do?PAGE_VALUE=1&ADMIN_MODULE=SUP&METHOD=USER_ADD&LOGIN=${login}');" value="Add Supplier">
            		</c:if>
            		<input name="Button2" type="button" class="buttonMain" onClick="javascript:self.close();" value="Close">
            	</c:when>
            	<c:otherwise>
            		<c:if test="${countLabel == 0}">
            			<input name="Button" type="button" class="buttonMain" onClick="javascript:checkvalidate('<%=request.getContextPath()%>/admin/adduserinfo.do?PAGE_VALUE=1&ADMIN_MODULE=SUP&METHOD=USER_EDIT&LOGIN=${login}');" value="Add Supplier">
            		</c:if>
            		<input name="Button2" type="button" class="buttonMain" onClick="javascript:self.close();" value="Close">
            	</c:otherwise>
            </c:choose>
