 ...
 *
 * Title		: 	DeliveryMessageWoodProcessorImpl.java
...
/**
 * DeliveryMessageWoodProcessorImpl is an implementation class to process the 
 * the DeliveryMessageWood Inbound Transaction XML for different vendors.
 * 
 * @author Ashish Agrawal
 */
public class DeliveryMessageWoodProcessorImpl implements IDeliveryMessageWoodProcessor{
	...
	public String processDeliveryMessageWood(InboundTransactionFactory inTransFactory, String xmlFileNameWithDir, String inDirArchiveXmlValid, String inDirArchiveXmlInvalid, String vendorSAN, TransactionStatusDTO transStatusDTO) {
		...
		IDeliveryMessageWoodDAO dmwDAO	= null;
		String status					= null;
		FileUtils fileUtils				= null;
		
		try {
			B2BLogger.debug("DeliveryMessageWoodProcessorImpl.processDeliveryMessageWood() method called");
			mapping = new Mapping();
			mapping.loadMapping(IPixB2BConstants.DMW_Mapping_XML);
			...
			if(pneDTO != null){
				dmwDAO = inTransFactory.getDeliveryMessageWoodDAO();
				status = dmwDAO.storeDeliveryMessageWoodDetails(pneDTO, transStatusDTO);
				fileUtils = new FileUtils();
				fileUtils.moveFile(xmlFileNameWithDir, inDirArchiveXmlValid);
			}else{
				B2BLogger.info("DeliveryMessageWoodProcessorImpl.processDeliveryMessageWood() - pneDTO = "+pneDTO);
			}
			B2BLogger.debug("DeliveryMessageWoodProcessorImpl.processDeliveryMessageWood() method return");
		} catch (IOException e) {
